...
class BinarySearchTree:
    ...
    def maximum(self) -> Optional[int]:
        ...
        if self.is_empty():
            ...
            if self._right is not None: #Find max
                value = self._right.maximum() #Keep going deeper and so on
    ...
    def count(self, item: Any) -> int:
        ...
        if self.is_empty():
            ...
            if item <= self._root: #Recurse through self.left
                counter = 0
                if self._root == item:
                    counter += 1
                if self._left is not None:
                    counter += self._left.count(item)
            else: #Recurse through self.right
                counter = 0
                if self._root == item:
                    counter += 1
    ...
    def items(self) -> List:
        ...
        [2, 3, 5, 7, 9, 11, 13]
        """
        #Get leftmost to the rightmost element
        lst = []
        if self.is_empty():
            return lst
        else:
    ...
    def smaller(self, item: Any) -> List:
        ...
        if self.is_empty():
            ...
            if item <= self._root: #Go left only
                if self._root < item: #check root
                    newlst.append(self._root)
                if self._left is not None: #go smaller
                    newlst.extend(self._left.smaller(item))
            else:
                newlst.extend(self._left.items())
                if self._root < item: #check root
                    newlst.append(self._root)
                if self._right is not None:
                    newlst.extend(self._right.smaller(item))
            return newlst
