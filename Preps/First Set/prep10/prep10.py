...
class BinOp(Expr):
    ...
    def __str__(self) -> str:
        ...
        >>> str(expr)
        '(10.5 + 30)'
        >>> ex = BinOp(Num(3), '+', Num(4))
        >>> str(ex)
        '(3 + 4)'
        >>> ex1 = BinOp(BinOp(Num(3), '+', Num(4)), '*', Num(6))
        >>> str(ex1)
        '((3 + 4) * 6)'
        >>> ex2 = BinOp(Num(1.1), '+', BinOp(Num(2.2), '+', BinOp(Num(3.3), '+', Num(4.4))))
        >>> str(ex2)
        '(1.1 + (2.2 + (3.3 + 4.4)))'
        """
...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr.evaluate()
        False
        >>> expr2 = BoolOp('or', [Bool(True), Bool(True), Bool(False)])
        >>> expr2.evaluate()
        True
        """
        if self.op == 'and':
            for boolz in self.values:
                if boolz.evaluate() is False:
                    return False
            return True
        else:
            for boolz in self.values:
                if boolz.evaluate() is True:
                    return True
            return False
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        for comp in self.comparisons:
            if comp[0] == '<=':
                expression = self.left.n <= comp[1].n
            elif comp[0] == '<':
                expression = self.left.n < comp[1].n
            elif comp[0] == '>=':
                expression = self.left.n >= comp[1].n
                ...
                expression = self.left.n == comp[1].n
            operation.values.append(Bool(expression))
