...
def find_runs(lst: list) -> List[Tuple[int, int]]:
    ...
    while run_end < len(lst):
        ...
        if lst[run_start] <= lst[run_end]: #if they check out
            run_start += 1
            run_end += 1
            if run_end == len(lst):
                runs.append((run_initial, run_end))
        else: #if they dont check out stop.
            runs.append((run_initial, run_end))
            run_start, run_initial = run_end, run_end
            run_end += 1
            if run_end == len(lst):
                runs.append((run_initial, run_end))
    return runs
