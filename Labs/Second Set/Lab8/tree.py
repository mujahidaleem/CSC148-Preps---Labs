...
class Tree:
    ...
    def branching_factor(self) -> float:
        #Average number of subtrees for each internal value
        #If internal value, get subtrees, add up all internal values, divide!
        ...
        if not self.is_empty():
            int_v, num_c = self.branching_factor_helper()
            return num_c/int_v
        return 0.0

    def branching_factor_helper(self) -> Tuple:
        '''gets children and internal values
                '''
        if self._subtrees == []:
            return 0, 0
        else:
            children = len(self._subtrees)
            internal_values = 1
            for subtree in self._subtrees:
                internal_v, number_of_children = \
                    subtree.branching_factor_helper()
                children += number_of_children
                internal_values += internal_v
            return internal_values, children
    ...
    def insert(self, item: Any) -> None:
        ...
        if self.is_empty():
            ...
            if random_number == 3:
                ...
                random_tree = random.choice(self._subtrees)
                for subtree in random_tree._subtrees:
                    subtree.insert(item)
