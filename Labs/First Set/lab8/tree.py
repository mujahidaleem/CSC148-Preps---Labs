...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        if not self.is_empty():
            intv, numc = self.branching_factor_helper()
            return numc / intv
        return 0.0

    def branching_factor_helper(self) -> Tuple:
        '''x,y where y is the number of children, and x is if an internal value
        '''
        if self.is_empty():
            return 0, 0
        elif self._subtrees == []:
            ...
            for subtree in self._subtrees:
                ...
                v, x = subtree.branching_factor_helper()
                children += x
                internal_values += v
    ...
    def items_at_depth(self, d: int) -> List:
        ...
        if self.is_empty():
            ...
            x = []
            for subtree in self._subtrees:
                x.extend(subtree.items_at_depth(d-1))
        return x
